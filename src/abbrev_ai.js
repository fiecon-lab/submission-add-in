...
$(document).ready(function() {
    ...
    function findAbbreviations() {
      return Word.run(async (context) => {
        ...
        // ----- New code to call ollama API to fill in missing definitions -----
        const pairsText = sortedAbbreviations.map(abbr => `${abbr}: ${definitions[abbr] || "[Missing]"}`).join('\n');
        const prompt = "Fill in the gaps for the following abbreviation definitions. If any definition is missing, please provide a suggestion:\n" + pairsText;
        
        try {
          const apiResponse = await $.ajax({
              url: "https://cria-api.fiecon.com/api/generate",
              method: "POST",
              headers: { "APIKey": "0a2e6ef6-4a96-406f-888e-865a8c5a7209" },
          ...
          console.log("Ollama API response:", apiResponse);
        } catch (error) {
